...
function App() {
  ...
  const addInterpretation = (quote, newInterpretation) => {
    ...
    const storageInterpretations = JSON.stringify([
      ...savedInterpretations,
      newInterpretationObj,
    ]);
  ...
  const checkLocalStorage = () => {
    ...
    if (localStorage.interpretations) {
      const retrievedInterpretations = JSON.parse(
  ...
  return (
    <div className="App">
      ...
      <Switch>
          <Route exact path="/" render={() => <DashboardIndex />} />
         {" "}
        <Route
          exact
          path="/category/:type"
        ...
        <Route
          exact
          path="/my-interpretations"
          render={() => (
            <IntepretationsSubmisssionsIndex
              favorites={favorites}
        ...
        <Route
          ...
          render={(match) => (
            <InterpretationIndex
              ...
              match={match}
              isEditing={true}
              editInterpretation={editInterpretation}
            />
          )}
        />
