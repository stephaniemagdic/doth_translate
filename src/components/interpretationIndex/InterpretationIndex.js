...
const InterpretationIndex = ({
  ...
  const findQuoteFromStorageById = () => {
    ...
    const newList = retrievedItems.filter(
      (quote) => parseInt(quote.id) === parseInt(quoteId)
    );
    if (!newList) {
    ...
    <div className="InterpretationIndex">
      {!isEditing && (
        <>
          ...
          <div className="quote-container">
            <nav>
              <button onClick={() => fetchANewQuote()}>GET A NEW QUOTE</button>
              <Link to="/">
                <button>CHOOSE A NEW CATEGORY</button>
              </Link>
            </nav>
            {quote && (
              <Quote
          ...
          <InterpretationInput
            handleSubmitInterpretation={handleSubmitInterpretation}
      ...
      {isEditing && (
        <>
          ...
          <div className="quote-container">
            <nav>
              <Link to="/doth_translate/">
                <span>
                  <button className="arrow-transition-button">
          ...
          <InterpretationInput
            handleSubmitEditInterpretation={handleSubmitEditInterpretation}
